...
const pageApi = {
  getBottomIconModuleList: { url: `${ctxPath}/bottomIcon/getBottomIconModuleList`, method: "RequestGet" } // 获取底部图标组件数据
}
...
Page({
  ...
  getBottom(cb) {
    RequestManager.RequestGet(pageApi.getBottomIconModuleList.url, {
      channel_id: getApp().globalData.configPageId
    },  (res) => {
      cb(res)
    }, (err) => { // error
  ...
  onLoad: function (options) {
    ...
    const getPageData = getApp().globalData.configPageId ? this.getBottom :common.gethomePageData
    getPageData((res) => {
